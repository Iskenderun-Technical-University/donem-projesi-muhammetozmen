...
from PyQt5 import QtCore, QtGui, QtWidgets #Arayüz modülleri
from PyQt5.QtWidgets import QFileDialog, QMainWindow #Arayüz modülleri
import shutil #Kopyalama vs işlemleri için kullanılır
import os #İşletim sistemi bilgilerini aktarır, dizin öğrenme için kullanıldı.

current_dir = os.getcwd() #Programın çalıştığı dizini kaydeder.

class Ui_page_main(object): #Arayüz objeleri
    def setupUi(self, page_main):
        ...
        #ENTER TEXT BUTTON: Kullanıcının girdiği yazıyı metne kaydedilmesi için fonksiyonu tetikler
        ...
        #DATA PATH ARAMA (KLASÖR): Kullanıcının sonradan seçmesi üzere dosya oluşturmasını sağlayan fonksiyonu tetikler
        ...
        #DATA PATH'A DOSYA OLUŞTUR: KUllanıcı eğer isterse deneme amaçlı doldurulmuş tablo oluşturabilir. (Hocam bunu kullanın mutlaka)
        ...
        #DATA PATH ARAMA (DOLDURULMUŞ DOSYA): Doldurulmuş dosyayı seçmenizi sağlayan fonksiyonu tetikler.
        ...
        #DATA PATH ARAMA 2 (KAYIT NOKTASI): İşlem sonucunun nereye kaydedileceğini seçmeye yaran fonksiyonu çağırır
        ...
        #FONT BÜYÜKLÜĞÜ'nü ayarlamayı sağlar
        ...
        #DÖNME BÜYÜKLÜĞÜ'nü ayarlamayı sağlar
        ...
        #BÜYÜLTME/KÜÇÜLTME BÜYÜKLÜĞÜ'nü ayarlamayı sağlar
        ...
        #YAZDIR BUTONU: Tüm işlemleri sonlandırır ve yazdırma işlemi için mainle bağlantıya geçer.
...
#BUTONLARA ATANAN GÖREVLER BURADAN SONRA BAŞLIYOR
    ...
    def entertext_button_clicked(self): #Girilen yazıyı kaydeder
    ...
    def data_path_browse_button_clicked(self): #Yazı tablosu için konum belirler
    ...
    def inputdata_path_browse_button_clicked(self): #Dolu yazı tablosunun konumunu alır
    ...
    def data_path_browse_button_2_clicked(self): #İşlem bittiğinde kaydedilecek dizini belirler
    ...
    def print_clicked(self): #YAZDIRMA ANA BUTON
        ...
        #Ana fonksiyona tüm bu değerleri yollar
    ...
    def retranslateUi(self, page_main): #Translate kısmı, QT designer ile oluşturuldu ve metin kısmını içinde tutuyor varsayılan dil için
...
def close_ui(): #Arayüzü kapatır
    import sys
    sys.exit()

def save_txt(): #Kullanıcının girdiği verileri main'den çekilmesi için kağıda döker
        global all_gui_values
...
#DEFAULT DEĞERLER
all_gui_values= tuple() #Arayüz verileri
selected_type= 'PNG' #Varsayılan kaydetme türü
font_size= 35 #Varsayılan font büyüklüğü
rotating_size= 10 #Varsayılan döndürme oranı (-10,10) olur
resizing_size= 10 #Varsayılan yeniden boyutlandırma (-0.1,0.1)
main_text_submit= 'Metin boş bırakıldı' #Varsayılan metin
data_folder_path= current_dir+'/img/tesdata' #Varsayılan harf tablosu kayıt konumu
inputdata_folder_path= current_dir + '/img/testdata/letter_table.png' #Varsayılan harf tablosu konumu
data_folder_path2= current_dir + '/img/workbench' #Varsayılan kayıt konumu

def start_gui(): #Arayüzü başlatma
